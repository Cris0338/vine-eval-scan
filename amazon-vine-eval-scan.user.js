// ==UserScript==
// @name         Amazon Vine FR — Scann Éval — "Light"
// @namespace    https://tampermonkey.net/
// @version      2.1.0
// @description  Light version: Scans only evaluations from the current Vine evaluation period (date captured from /vine/account). Stops automatically when reaching older reviews. Simplified UI with "Scann" button and "Reset dati" link.
// @author       Cris0338 (modified)
...
(() => {
  ...
  const LS_KEY_STATE = `vine_eval_counts_${HOST}_completed_light`;   // Stored counts and scan info
  const LS_KEY_PERIOD = `vine_eval_period_${HOST}`;                  // Period start date from account page
  const NP_IF_EN_ATTENTE_OLDER_THAN_DAYS = 180;                      // Treat old "en attente" as N.P.
  ...
  const emptyCounts = () => ORDER.reduce((acc, k) => (acc[k] = 0, acc), {});
  ...
  function parseDDMMYYYY(s) {
    ...
    // French textual format: "05 sept. 2025"
    const months = {
  ...
  function tryCapturePeriodOnAccountPage() {
    const el = document.getElementById('vvp-evaluation-period-tooltip-trigger');
    if (!el) return false;
  ...
  function scoreClass(score) {
    if (score === null) return 'vineScoreNA';
    if (score < 2) return 'vineScoreLow';
    if (score < 3) return 'vineScoreMid';
    return 'vineScoreGood';
  }
  ...
  GM_addStyle(`
    ...
    #vineEvalWrapper{width:100% !important;margin-top:10px;}
    #vineEvalFlex{display:flex;gap:12px;align-items:stretch;width:100%;}
    @media (max-width:1100px){#vineEvalFlex{flex-direction:column;}}
    #vineEvalTableWrap{flex:1;border-radius:12px;overflow:hidden;border:1px solid rgba(0,113,133,.35);background:rgba(255,255,255,0.02);}
    #vineEvalTableScroll{overflow-x:auto;}
    #vineEvalTable{width:100%;min-width:980px;border-collapse:separate;border-spacing:0;font-size:13px;}
    #vineEvalTable thead th{background:#007185;color:#fff;text-align:center;padding:10px;font-weight:800;}
    ...
    #vineEvalTable tbody td{padding:10px;text-align:center;border-top:1px solid rgba(255,255,255,0.08);font-variant-numeric:tabular-nums;}
    ...
    .vineEvalDelta{display:inline-block;margin-left:6px;font-weight:900;color:#10c010;}
    .vineEvalMetaLine{display:flex;gap:10px;flex-wrap:wrap;align-items:center;font-size:12px;opacity:.9;}
    .vineEvalStatus{font-size:12px;font-weight:700;opacity:.95;}
    .vineResetLink{font-size:11px;color:#007185;cursor:pointer;margin-top:4px;}
    .vineResetLink:hover{text-decoration:underline;}
    #vineEvalScoreWrap{flex:0 0 240px;min-width:240px;border-radius:12px;overflow:hidden;border:1px solid rgba(0,113,133,.35);background:rgba(255,255,255,0.02);}
    @media (max-width:1100px){#vineEvalScoreWrap{flex:1;min-width:0;}}
    #vineEvalScoreHead{background:#007185;color:#fff;padding:10px 12px;font-weight:800;display:flex;justify-content:space-between;align-items:center;}
    #vineEvalScoreHead span{opacity:.95;font-weight:700;}
    #vineEvalScoreBody{padding:12px;}
    .vineScoreLabel{font-size:12px;font-weight:800;opacity:.85;margin-bottom:6px;}
    .vineScoreValueRow{display:flex;align-items:baseline;justify-content:space-between;gap:10px;}
    .vineScoreValue{font-size:28px;font-weight:900;letter-spacing:.2px;font-variant-numeric:tabular-nums;line-height:1;}
    .vineScoreMax{opacity:.65;font-weight:800;font-size:12px;}
    .vineScoreHint{margin-top:6px;font-size:12px;opacity:.75;font-weight:700;}
    .vineScoreNA .vineScoreValue{color:rgba(255,255,255,.55);}
    .vineScoreLow .vineScoreValue{color:#ff9b2f;}
    .vineScoreMid .vineScoreValue{color:#ffd84a;}
    .vineScoreGood .vineScoreValue{color:#26d926;}
  `);
  ...
  function mountUIOnce() {
    ...
    if (!document.getElementById('vine-eval-scan-btn')) {
      ...
      btn.innerHTML = `<span class="a-button-inner"><a href="javascript:void(0)" class="a-button-text">Scann</a></span>`;
    ...
    if (!document.getElementById('vineEvalWrapper')) {
      ...
      wrapper.innerHTML = `
        <div id="vineEvalFlex">
                ...
                <tbody>
                  <tr id="vineEvalRowPeriod">
                    ...
                    <td>
                      ...
                      <div class="vineResetLink" id="vineResetLink">Reset</div>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div id="vineEvalScoreWrap">
            <div id="vineEvalScoreHead"><div>Score pondéré</div><span>/4</span></div>
            <div id="vineEvalScoreBody">
              <div class="vineScoreBlock vineScoreNA" id="vineScorePeriod">
                <div class="vineScoreLabel">Depuis éval</div>
                <div class="vineScoreValueRow">
                  <div class="vineScoreValue" id="vineScorePeriodVal">—</div>
                  <div class="vineScoreMax" id="vineScorePeriodDen">0 avis</div>
                </div>
      ...
      // Attach reset link
      document.getElementById('vineResetLink').addEventListener('click', resetAllData);
    }
  ...
  function fillRow(counts, deltas) {
    for (const k of ORDER) {
      ...
      const delta = (deltas && deltas[k] > 0) ? ` <span class="vineEvalDelta">+${deltas[k]}</span>` : '';
      el.innerHTML = `<b>${val}</b>${delta}`;
    }
  }
  ...
  function updateScoreCard(st) {
    const period = loadPeriod();
    const wrap = document.getElementById('vineScorePeriod');
    const valEl = document.getElementById('vineScorePeriodVal');
    const denEl = document.getElementById('vineScorePeriodDen');
    const hintEl = document.getElementById('vineScorePeriodHint');

    if (!period) {
      wrap.className = 'vineScoreBlock vineScoreNA';
      valEl.textContent = '—';
      denEl.textContent = '0 avis';
      hintEl.textContent = 'Va sur "Compte Vine"';
      return;
    }

    const { score, total } = computeWeightedScore(st.counts);
    wrap.className = `vineScoreBlock ${scoreClass(score)}`;
    valEl.textContent = score === null ? '—' : score.toFixed(2);
    denEl.textContent = `${total} avis`;
    hintEl.textContent = scoreLabel(score);
  }
  ...
  function render(st) {
    ...
    if (headLast) headLast.innerHTML = `Dernier Scann: <b>${formatFR(st.lastScanAt)}</b>`;
    ...
    const itemHtml = itemDelta > 0 ? ` <span class="vineEvalDelta">(+${itemDelta})</span>` : '';
    const pageHtml = pageDelta > 0 ? ` <span class="vineEvalDelta">(+${pageDelta})</span>` : '';

    meta.innerHTML = `
      <span>Total: <b>${st.scannedItems}</b>${itemHtml}</span>
      <span>| Pages: <b>${st.pagesScanned}</b>${pageHtml}</span>
    `;
  ...
  function extractFromDoc(doc, startTs) {
    ...
    for (const row of rows) {
      ...
      const tsAttr = row.querySelector('td[data-order-timestamp]')?.getAttribute('data-order-timestamp');
      const ts = tsAttr ? Number(tsAttr) : NaN;
      if (!Number.isFinite(ts) || ts < startTs) continue; // Skip reviews before period

      const klass = classifyEval(evalCell.textContent, getRowAgeDays(ts));
      if (klass && counts[klass] !== undefined) {
        counts[klass]++;
        items++;
      }
    }
  ...
  function looksBlocked(doc) {
    ...
    return /captcha|robot|saisissez les caractères|identifiez-vous|connexion.*mot de passe/i.test(txt);
  }
  ...
  async function scanAllPages() {
    const period = loadPeriod();
    if (!period) {
      document.getElementById('vineEvalStatus')?.setAttribute('textContent', 'Période non définie. Va sur "Compte Vine".');
      return;
    }
    ...
    try {
      ...
      // Page 1 (current)
      let { items, counts } = extractFromDoc(document, startTs);
      if (items === 0) {
      ...
      Object.keys(counts).forEach(k => st.counts[k] += counts[k]);
      ...
      for (let p = 2; p <= cap; p++) {
        ...
        ({ items, counts } = extractFromDoc(doc, startTs));
        if (items === 0) {
          emptyStreak++;
          if (looksBlocked(doc)) {
