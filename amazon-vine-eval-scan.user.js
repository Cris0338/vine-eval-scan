...
(() => {
  ...
  function savePeriod(startDate, startTs) {
    localStorage.setItem(
      LS_KEY_PERIOD,
      JSON.stringify({ startDate, startTs, savedAt: new Date().toISOString() }),
    );
  }
  ...
  function captureReviewEditorSnapshot() {
    ...
    const rating =
      ...
      document
        .querySelector(
          '[role="radio"][aria-label*="étoile"][aria-checked="true"]',
        )
        ?.getAttribute("aria-label") ??
      document
        .querySelector(
          '[role="radio"][aria-label*="star"][aria-checked="true"]',
        )
        ?.getAttribute("aria-label") ??
      "";
    ...
    const media = Array.from(
      document.querySelectorAll(
        '[data-hook*="media"] img[src], [data-hook*="media"] video[src], [data-testid*="media"] img[src], [data-testid*="media"] video[src], [class*="media"] img[src], [class*="media"] video[src]',
      ),
    )
  ...
  if (PATH.startsWith("/vine/account")) {
    GM_addStyle(
      `#vinePeriodOverlay{position:fixed;inset:0;z-index:999999;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center}#vinePeriodOverlay .vine-period-card{width:min(560px,calc(100% - 24px));border-radius:14px;padding:18px;background:#111;border:1px solid rgba(255,255,255,.14);box-shadow:0 12px 40px rgba(0,0,0,.45);text-align:center}#vinePeriodOverlay .vine-period-title{font-size:14px;opacity:.9;margin-bottom:10px;color:#fff}#vinePeriodOverlay .vine-period-date{font-size:26px;font-weight:800;color:#1fbf1f;letter-spacing:.4px;font-variant-numeric:tabular-nums}`,
    );
  ...
  function isNonApprovedStatus(text) {
    return /(?:^|\s)non approuvee?(?:\s|$|[.:;!,(])/i.test(
      normalizeLoose(text),
    );
  }
  function findColumnIndexes(doc) {
    ...
    const headers = Array.from(
      headingRow?.querySelectorAll(":scope > th") || [],
    );
    let status = headers.findIndex(
      (h) => h.id === "vvp-reviews-table--review-content-heading",
    );
    let evaluation = headers.findIndex(
  ...
  function extractAsinFromRow(row) {
    ...
    const m = (
      (titleCell ? titleCell.textContent : row.textContent) || ""
    ).match(/\(([A-Z0-9]{10})\)/i);
    return m ? m[1].toUpperCase() : null;
  }
  ...
  function extractProductMeta(row, ts) {
    const image = row.querySelector("td.vvp-reviews-table--image-col img, img"),
      textCell = row.querySelector("td.vvp-reviews-table--text-col"),
      productLink = textCell?.querySelector(
        'a[href*="/dp/"], a[href*="/gp/product/"]',
      ),
      firstTextLink = textCell?.querySelector("a"),
      tsCell = row.querySelector("td[data-order-timestamp]");
    return {
      productTitle:
      ...
      productUrl: productLink
        ? absoluteAmazonUrl(productLink.getAttribute("href"))
        : null,
      imageUrl:
    ...
    for (const row of rows) {
      ...
      const reviewStatusCell = cellAt(row, columns.status),
        ...
        statusText = (reviewStatusCell?.textContent || "")
          ...
          .replace(/\s+/g, " ")
          .trim(),
        approvalPending = isApprovalPendingStatus(statusText),
        nonApproved = isNonApprovedStatus(statusText);
      const state = classifyEval(
  ...
  function isValidReviewsDoc(doc) {
    return (
      !!doc && doc.querySelectorAll("tr.vvp-reviews-table--row").length > 0
    );
  }
  function pageHasAnyInPeriod(doc, startTs) {
    return Array.from(
      doc.querySelectorAll(
        "tr.vvp-reviews-table--row td[data-order-timestamp]",
      ),
    ).some((c) => {
  ...
  function iframeEditabilityCheck(editUrl) {
    return new Promise((resolve) => {
      ...
      iframe.style.cssText =
        "position:fixed!important;width:1px!important;height:1px!important;left:-10000px!important;top:-10000px!important;opacity:0!important;pointer-events:none!important;border:0!important;";
      let finished = false,
        poll = null;
      const finish = (result) => {
        if (finished) {
      ...
      setTimeout(
        () => finish({ editability: "unknown", editResolvedUrl: editUrl }),
        12000,
      );
      iframe.src = editUrl;
    });
  ...
  async function enrichNonApproved(entry, oldRecord = null) {
    const base = {
      ...
      visited: !!oldRecord?.visited,
      visitedAt: oldRecord?.visitedAt || null,
      manualModifiable: !!oldRecord?.manualModifiable,
      manualModifiableAt: oldRecord?.manualModifiableAt || null,
      manualVerified: !!oldRecord?.manualVerified,
      manualVerifiedAt: oldRecord?.manualVerifiedAt || null,
    };
    ...
    if (oldRecord?.manualModifiable) {
      base.editability = "modifiable";
      return base;
    }
    if (
      oldRecord?.manualVerified &&
      oldRecord?.editability === "non-modifiable"
    ) {
      base.editability = "non-modifiable";
      return base;
    }
    if (!entry.editUrl) {
  ...
  GM_addStyle(
    `#vvp-review-button-container{display:flex!important;align-items:center!important;gap:6px!important;flex-wrap:wrap!important}#vineEvalWrapper{width:100%!important;margin-top:10px;box-sizing:border-box}#vineEvalFlex{display:flex;gap:12px;align-items:flex-start;width:100%;box-sizing:border-box}@media(max-width:1100px){#vineEvalFlex{flex-direction:column;align-items:stretch}}#vineEvalTableWrap{flex:1 1 auto;min-width:0;height:106px;display:flex;flex-direction:column;border-radius:12px;overflow:hidden;border:1px solid rgba(0,113,133,.35);background:rgba(255,255,255,.02);box-sizing:border-box}#vineEvalTableScroll{overflow-x:auto;min-width:0;box-sizing:border-box;flex:1 1 auto}#vineEvalTable{width:100%;height:100%;border-collapse:separate;border-spacing:0;font-size:13px;table-layout:fixed}#vineEvalTable thead th{background:#007185;color:#fff;text-align:center;padding:10px;font-weight:800;white-space:nowrap}#vineEvalTable thead th:first-child,#vineEvalTable thead th:last-child{text-align:left;padding-left:12px;padding-right:12px}#vineEvalTable tbody td{padding:10px;text-align:center;border-top:1px solid rgba(0,0,0,.08);font-variant-numeric:tabular-nums;vertical-align:middle;white-space:nowrap}#vineEvalTable tbody td:first-child,#vineEvalTable tbody td:last-child{text-align:left;padding-left:12px;padding-right:12px;white-space:normal}.vineEvalHeadFlex{display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%}.vineEvalHeadFlex .vineHeadRight{text-align:right;font-weight:700;opacity:.95}.vineEvalRowTitle{font-weight:900}.vineEvalMuted{opacity:.7}.vineEvalDeltaPos{display:inline-block;margin-left:6px;font-weight:900;color:#10c010}.vineEvalDeltaNeg{display:inline-block;margin-left:6px;font-weight:900;color:#ff4d4d}.vineEvalMetaLine{display:flex;gap:10px;flex-wrap:nowrap;align-items:center;font-size:12px;opacity:.9;width:100%}.vineEvalMetaSpacer{flex:1;min-width:0}.vineEvalStatus{font-size:12px;font-weight:700;opacity:.95}.vineRefreshDots{display:inline-flex;margin-left:2px}.vineRefreshDots span{display:inline-block;animation:vineRefreshDot 1.2s infinite;opacity:.2}.vineRefreshDots span:nth-child(2){animation-delay:.2s}.vineRefreshDots span:nth-child(3){animation-delay:.4s}@keyframes vineRefreshDot{0%,20%,100%{opacity:.2}50%{opacity:1}}.vineResetLink{font-size:11px;color:#007185;cursor:pointer;white-space:nowrap;user-select:none}.vineResetLink:hover{text-decoration:underline}#p-non-approved a{color:#007185;text-decoration:none;font-weight:900}#p-non-approved a:hover{text-decoration:underline}#vineEvalScoreWrap{flex:0 0 240px;min-width:240px;height:106px;border-radius:12px;overflow:hidden;border:1px solid rgba(0,113,133,.35);background:rgba(255,255,255,.02);align-self:flex-start;box-sizing:border-box}@media(max-width:1100px){#vineEvalScoreWrap{flex:1;min-width:0;align-self:stretch}}#vineEvalScoreHead{background:#007185;color:#fff;padding:8px 10px;font-weight:800;display:flex;justify-content:space-between;align-items:center}.vineScoreHeadRight{display:flex;align-items:baseline;gap:6px}.vineScoreHeadVal{font-size:22px;font-weight:900;color:#26d926;line-height:1}.vineScoreHeadMax{opacity:.9;font-weight:800}#vineEvalScoreBody{padding:10px;display:flex;flex-direction:column;gap:6px}.vineScoreMetaRow{display:flex;align-items:center;justify-content:space-between;gap:10px}.vineScoreLabel,.vineScoreDen{font-size:12px;font-weight:800;opacity:.8}.vineScoreHint{font-size:14px;font-weight:900}.vineScoreExcellent{color:#26d926!important}#vineNonApprovedPage{margin:16px 0 28px;border:1px solid #d5d9d9;border-radius:12px;overflow:hidden;background:#fff;color:#111}#vineNonApprovedPage .nap-head{display:flex;align-items:center;justify-content:space-between;gap:12px;background:#007185;color:#fff;padding:14px 16px}#vineNonApprovedPage .nap-head h2{margin:0;font-size:20px}#vineNonApprovedPage .nap-back{color:#fff;text-decoration:underline;font-weight:700}#vineNonApprovedPage .nap-section{padding:16px}#vineNonApprovedPage .nap-section h3{margin:0 0 12px;font-size:18px}#vineNonApprovedPage .nap-separator{height:8px;background:linear-gradient(90deg,#007185,#d5d9d9,#007185);margin:4px 0;border-top:1px solid #007185;border-bottom:1px solid #007185}#vineNonApprovedPage .nap-table-wrap{overflow-x:auto}#vineNonApprovedPage table{width:100%;border-collapse:collapse;min-width:820px}#vineNonApprovedPage th{background:#f0f2f2;text-align:left;padding:9px;border-bottom:1px solid #d5d9d9;white-space:nowrap}#vineNonApprovedPage td{padding:9px;border-bottom:1px solid #eee;vertical-align:middle}#vineNonApprovedPage .nap-img{width:64px;height:64px;object-fit:contain}#vineNonApprovedPage .nap-action{display:inline-block;padding:6px 10px;border-radius:16px;background:#ffd814;border:1px solid #fcd200;color:#111;text-decoration:none;font-weight:700;white-space:nowrap}#vineNonApprovedPage .nap-verify{background:#fff;border-color:#888}#vineNonApprovedPage .nap-visited{background:#f3f3f3!important;border-color:#888!important;color:#555!important}#vineNonApprovedPage .nap-verified{background:#edf7ed!important;border-color:#067d62!important;color:#067d62!important}#vineNonApprovedPage .nap-modified{font-weight:900;color:#067d62}#vineNonApprovedPage .nap-promote{display:flex;align-items:center;gap:6px;white-space:nowrap;font-size:12px}#vineNonApprovedPage .nap-promote input{width:16px;height:16px;cursor:pointer}#vineNonApprovedPage .nap-empty{padding:12px;border:1px dashed #aaa;border-radius:8px;opacity:.75}`,
  );
  ...
  function mountUIOnce() {
    ...
    if (!document.getElementById("vine-eval-scan-btn")) {
      ...
      btn.innerHTML =
        '<span class="a-button-inner"><a href="javascript:void(0)" class="a-button-text">Scan</a></span>';
    ...
    if (!document.getElementById("vineEvalWrapper")) {
      ...
      wrapper.innerHTML = `<div id="vineEvalFlex"><div id="vineEvalTableWrap"><div id="vineEvalTableScroll"><table id="vineEvalTable"><thead><tr><th style="width:260px;">Période</th><th style="width:170px;">En attente d'approbation</th><th>En attente</th><th>Excellent</th><th>Bien</th><th>Juste</th><th>Pauvre</th><th style="width:155px;">Non approuvé</th><th style="width:360px;"><div class="vineEvalHeadFlex"><span>Infos</span><span class="vineHeadRight" id="vineEvalHeadLastScan">Dernier scan: —</span></div></th></tr></thead><tbody><tr id="vineEvalRowPeriod"><td class="vineEvalRowTitle" id="vineEvalPeriodTitle">Depuis éval</td><td id="p-approval-pending"></td><td id="p-en-attente"></td><td id="p-excellent"></td><td id="p-bien"></td><td id="p-juste"></td><td id="p-pauvre"></td><td id="p-non-approved"></td><td><div class="vineEvalStatus" id="vineEvalStatus"></div><div class="vineEvalMetaLine" id="vineEvalMetaPeriod"></div></td></tr></tbody></table></div></div><div id="vineEvalScoreWrap"><div id="vineEvalScoreHead"><div>Score pondéré</div><div class="vineScoreHeadRight"><span class="vineScoreHeadVal" id="vineScoreHeadVal">—</span><span class="vineScoreHeadMax">/4</span></div></div><div id="vineEvalScoreBody"><div class="vineScoreMetaRow"><div class="vineScoreLabel">Depuis éval</div><div class="vineScoreDen" id="vineScorePeriodDen">0 avis notés</div></div><div class="vineScoreHint" id="vineScorePeriodHint">—</div></div></div></div>`;
  ...
  function fillNonApproved(st) {
    ...
    const blocked =
      ...
      Number(st.nonApprovedUnknown || 0);
    el.innerHTML = `<a href="${nonApprovedPageUrl()}" title="Voir les Non approuvés">${mod} ✏️</a> (${blocked} 🚫)`;
  }
  function computeWeightedScore(counts) {
    const ex = counts.excellent ?? 0,
      ...
      total = ex + bi + ju + pa;
    return total
      ? { score: (4 * ex + 3 * bi + 2 * ju + pa) / total, total }
      : { score: null, total: 0 };
  }
  function render(st) {
    ...
    if (!period) {
      ...
      const deltas = computeDeltas(st.counts, st.prevCounts),
        approvalDelta =
          typeof st.prevPendingApproval === "number"
            ? (st.pendingApproval ?? 0) - st.prevPendingApproval
            : null;
      fillApprovalPending(st.pendingApproval, approvalDelta);
  ...
  function renderNonApprovedPage() {
    const map = loadAsinMap();
    const modifiable = Object.values(map)
      .filter((r) => r?.nonApproved && r.editability === "modifiable")
      .sort((a, b) => (b.orderTs || 0) - (a.orderTs || 0));
    const blocked = Object.values(map)
      .filter((r) => r?.nonApproved && r.editability !== "modifiable")
      .sort((a, b) => (b.orderTs || 0) - (a.orderTs || 0));
    ...
    if (!root) {
      ...
      document
        .getElementById("vineEvalWrapper")
        ?.insertAdjacentElement("afterend", root);
    }
    if (!root.dataset.handlers) {
      ...
      root.addEventListener(
        ...
        (e) => {
          ...
          rec.editability = "modifiable";
          rec.checkedAt = now;
          latest[key] = rec;
          saveAsinMap(latest);
          const st = loadState();
          applyNonApprovedTotalsFromMap(st, latest);
          saveState(st);
          render(st);
        },
        true,
    ...
    const product = (r) =>
      r.productUrl
        ? `<a href="${escapeHtml(r.productUrl)}" target="_blank" rel="noopener">${escapeHtml(r.productTitle || "Produit")}</a>`
        : escapeHtml(r.productTitle || "Produit");
    const image = (r) =>
      r.imageUrl
  ...
  async function scanAllPages() {
    ...
    st.prevPendingApproval =
      typeof prev.pendingApproval === "number" ? prev.pendingApproval : null;
  ...
  async function refreshPending() {
    ...
    const pendingTargets = new Set(
        Object.entries(asinMap)
          .filter(
            ([, v]) => v && (v.pending || v.approvalPending || v.nonApproved),
          )
          .map(([k]) => k),
      ),
      seen = new Set();
    ...
    try {
      ...
      while (
        page <= maxPagesFailsafe &&
        pendingTargets.size > 0 &&
        outOfPeriodStreak < 2
      ) {
        if (status) {
          status.innerHTML = `Refresh page ${page} <span class="vineRefreshDots" aria-hidden="true"><span>.</span><span>.</span><span>.</span></span>`;
        }
          ...
          for (let attempt = 1; attempt <= 3; attempt++) {
            try {
              const { text } = await httpGet(pageUrl(page)),
                tmp = new DOMParser().parseFromString(text, "text/html");
              if (isValidReviewsDoc(tmp)) {
                doc = tmp;
                break;
              }
        ...
        for (const entry of entries) {
          ...
          if (old) {
            ...
            if (old.state !== info.state) {
              st.counts[old.state] = Math.max(
                0,
                (st.counts[old.state] || 0) - 1,
              );
